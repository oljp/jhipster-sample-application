...
public interface SelRepository extends JpaRepository<Sel, Long> {
    ...
    @Query(value = "select distinct sel from Sel sel left join fetch sel.distinctEvents left join fetch sel.distinctEvents",
        countQuery = "select count(distinct sel) from Sel sel")
    Page<Sel> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct sel from Sel sel left join fetch sel.distinctEvents left join fetch sel.distinctEvents")
    List<Sel> findAllWithEagerRelationships();

    @Query("select sel from Sel sel left join fetch sel.distinctEvents left join fetch sel.distinctEvents where sel.id =:id")

...
public class DistinctEvent implements Serializable {
    ...
    @ManyToMany
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JoinTable(name = "distinct_event_sel",
               joinColumns = @JoinColumn(name = "distinct_event_id", referencedColumnName = "id"),
               inverseJoinColumns = @JoinColumn(name = "sel_id", referencedColumnName = "id"))
    private Set<Sel> sels = new HashSet<>();

    @ManyToMany(mappedBy = "initiatingEvents")
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JsonIgnore
    private Set<Sel> initiatingSels = new HashSet<>();

    @ManyToMany(mappedBy = "associatedEvents")
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    @JsonIgnore
    ...
    public Set<Sel> getSels() {
        return sels;
    }

    public DistinctEvent sels(Set<Sel> sels) {
        this.sels = sels;
        return this;
    }

    public DistinctEvent addSel(Sel sel) {
        this.sels.add(sel);
        sel.getDistinctEvents().add(this);
        return this;
    }

    public DistinctEvent removeSel(Sel sel) {
        this.sels.remove(sel);
        sel.getDistinctEvents().remove(this);
        return this;
    }

    public void setSels(Set<Sel> sels) {
        this.sels = sels;
    }
